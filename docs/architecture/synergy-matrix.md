...
## Actualización 2026-05-04 - Rehidratación Template Editor

1. `14-main-app` diferencia notebook origen y mirror `.docx` en Home: el riesgo cruzado es volver a abrir el DOCX por el lector de archivos en vez de tokenizarlo para `template_attach`.
2. `17-template-editor` consume ACKs con `template` completo como payload autoritativo y conserva selección por `style_id`/slot, evitando drift UI tras mutaciones de fuente o defaults globales.
3. `04-notebook-handlers` y `01-document-generation-docx` dependen de que `semantic_style_slots` sobreviva a reextracciones para que el kernel genere DOCX con estilos Word localizados.

---

## Actualización 2026-05-08 - Banco exhaustivo Template Editor

1. `17-template-editor` incorpora un banco reproducible con fixtures DOCX sintéticas, cobertura backend de extracción/mutación/sanitización y E2E Playwright de upload+attach, edición rápida/Word-complete, import/export y reopen.
2. `01-document-generation-docx` queda dentro del oráculo: el E2E genera un DOCX real desde notebook, inspecciona OOXML y ejecuta Workbench `audit`/`render_all_pages` para validar que la plantilla editada produce artefactos no vacíos y renderizables.
3. `14-main-app` y `04-notebook-handlers` quedan cubiertos en la ruta crítica de attach determinístico: el banco espía `template_attach` para detectar doble envío o pérdida por reconnect/pending antes de aceptar la corrida.

---

## Actualización 2026-05-08 - Plantilla JSON anidada al notebook

1. `04-notebook-handlers` aplica automáticamente `metadata.inspyro.template_binding` al crear, cargar o resetear un kernel; errores `missing`/`error` del JSON quedan en `template_binding` sin bloquear ejecución.
2. `17-template-editor` conserva el formato portable `schema_version=1.1` como unidad persistida y reexporta el JSON vinculado tras ACKs autoritativos de upload, slots, defaults, estilos y formato de tablas.
3. `14-main-app` promueve el binding del `.ipynb` sobre el índice legacy `.inspyro/templates/index.json`, muestra estados `Vinculada`/`Perdida`/`Sin plantilla` y ofrece migración desde Home o el editor.
4. `19-mcp-server` expone `bind_template_to_notebook` y propaga `template_binding` en load/create/info para que clientes IA usen el mismo flujo notebook-first que la UI.
5. `agent_debug.ps1 template-binding-bank` conecta estas cuatro piezas con un banco live que deja matriz requisito→escenario→resultado→artefactos bajo `output/template-binding-bank/`.
...
1. `04-notebook-handlers` expone `docx` como tipo lógico al cargar/ejecutar notebooks, pero persiste `.ipynb` estándar (`cell_type="code"` + `metadata.inspyro.cell_kind="docx"`), migra suavemente fuentes legacy con APIs DOCX detectables y sigue tratando `emit_docx` como modo documental de la corrida, no como alias exclusivo del tipo de celda.
